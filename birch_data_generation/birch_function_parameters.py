...
def MATLAB_definition():
    ...
    # % sets parameter values to variables
    # p = inputParser;

    # % Sets default values to parameters
    # addParameter(p, 'sizeVolume', [500,1200,300]);
    # addParameter(p, 'random_seed', 42);
    # addParameter(p, 'saveSlice', 1);
    # addParameter(p, 'cellR', 14.5);
    # addParameter(p, 'cellLength', 2341);
    # addParameter(p, 'cellLengthVariance', 581);
    # addParameter(p, 'rayCellLength', 62);
    # addParameter(p, 'rayCell_variance', 15);
    # addParameter(p, 'rayCellNum', 11.33);
    # addParameter(p, 'rayCellNumStd', 3.39);
    # addParameter(p, 'vesselLength', 780);
    # addParameter(p, 'vesselLengthVariance', 195);
    # addParameter(p, 'raywHeight', 42);
    # addParameter(p, 'raySpace', 20);
    # addParameter(p, 'isExistVessel', 1);
    # addParameter(p, 'isExistRayCell', 1);
    # addParameter(p, 'cellWallThick', 2);

    # parse(p, varargin{:});


    # %% Sets randomseed
    # seed = p.Results.random_seed;

    # rng(seed);

    # %% set parameters
    # % Volume structure size;
    # sizeVolume    = p.Results.sizeVolume; %[500,1200,300];
    ...
    # SaveFolder = save_folder_name;   %'SaveBirch/'; % Folder to save the data %Changes made
    # mkdir(SaveFolder);

    # saveSlice          = p.Results.saveSlice;

    # % cellwall2radiusRatio  = 1.38/(6.01+1.38);
    # cellR              = p.Results.cellR; % The grid distance for the nodes we generated. In Unit of voxels.
    ...
    # cellLength         = p.Results.cellLength;  % Average fiber length
    # cellLengthVariance = p.Results.cellLengthVariance;  % Standard deviation of fiber length
    ...
    # vesselLength       = p.Results.vesselLength; % Average vessel length
    # vesselLengthVariance = p.Results.vesselLengthVariance; % Standard deviation of vessel length

    # % ray cell basic length
    # rayCellLength      = p.Results.rayCellLength; % ray cell length along radial direction
    # % ray cell disturbance
    # rayCell_variance   = p.Results.rayCell_variance; % ray cell length deviation along radial direction
    # cellEndThick       = 4;  % End of cell wall thickness along L direction
    # cellWallThick      = p.Results.cellWallThick;  % Cell wall thickness
    # isExistVessel      = p.Results.isExistVessel;  % Include vessels, 1: with vessel, 0: no vessel
    # isExistRayCell     = p.Results.isExistRayCell;  % Is there any ray cells, 1: exist ray cells; 0: no ray cell
    ...
    # raywHeight         = p.Results.raywHeight; % the height of the ray cell along fiber direction
    # raySpace           = p.Results.raySpace; % the space between ray cell along T direction. The distance is raySpace*cellR
    # % four neighbor grids. Do not change here
    # neighborLocal      = [-1,  0, 1, 0;
    #                         0, -1, 0, 1];
    # % the grid node number
    # rayCellNum         = p.Results.rayCellNum; % ray cell count in a group of ray cells
    # rayCellNumStd      = p.Results.rayCellNumStd;  % ray cell count in a group of ray cells
    # VesselCount        = 50;   % This number is used to control the vessel number and distribution.
    # % Please tune with this parameter
    # writeLocalDeformData  = 1; % 1 or 0.
    ...
    #     writematrix(round(u,3),u_FileName)
    #     v_FileName = fullfile(Folder_dips_LocalDistV,FileVAll{z});
    #     if isExistRayCell
    #         writematrix(round(v+v_all_raycell,3),v_FileName)
    #     else
    #         writematrix(round(v,3),v_FileName)
    #     end
